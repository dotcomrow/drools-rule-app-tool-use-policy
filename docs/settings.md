...
| `REPO_VERSION` | no | `1.0.163` | config | Version to stamp into pom.xml after import. If empty or 'auto', derived from the repo. |
...
| `VERSION` | no | `1.0.163` | config | KJAR version to register in the container spec. If empty or 'auto', derived from the repo. |
| `VERSION_PREFIX` | no | `` | config | Prefix used when deriving a version from git history (e.g. 1.0.). Leave empty to infer from pom. |
