...
RISK_SCORE="${RISK_SCORE:-72}"
AMOUNT="${AMOUNT:-150.0}"
ACCOUNT_AGE_DAYS="${ACCOUNT_AGE_DAYS:-120}"
      ...
      "insert": {
        "object": {
          "com.dotcomrow.rules.tooluse.ToolUseRequest": {
            "tool": os.environ.get("TOOL", "refund"),
            "userRole": os.environ.get("USER_ROLE", "member"),
            "riskScore": int(os.environ.get("RISK_SCORE", "72")),
            "amount": float(os.environ.get("AMOUNT", "150.0")),
            "accountAgeDays": int(os.environ.get("ACCOUNT_AGE_DAYS", "120")),
          }
        },
...
with open("/tmp/tool-use-policy.json", "w") as handle:
